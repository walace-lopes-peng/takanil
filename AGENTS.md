...
- `supabase/schema.sql`, `supabase/schema_piloto.sql` e `supabase/migrations/` — **nunca alterar uma tabela existente sem eu pedir explicitamente**. Se a tarefa precisa de um campo novo, pare e pergunte antes de rodar qualquer SQL.
- **Paridade Obrigatória e Migrations Automatizadas (Supabase CLI):** O projeto utiliza `public` para testes/dev e `piloto` para dados reais de produção. Para não criar nem alterar tabelas "na unha", utilize sempre o fluxo de migrations versionadas do Supabase CLI (`supabase migration new <nome>` ou `supabase db diff --schema public,piloto`). Toda alteração DDL deve ser aplicada uniformemente em ambos os schemas para evitar Schema Drift.
...
- **Separação Obrigatória: Novidades vs. Melhorias e Correções (A Regra dos Três Baldes):**
  - **`### ✨ Novas Funcionalidades`:** Reservado estritamente para ferramentas e fluxos inéditos que o usuário **NÃO conseguia fazer antes** e agora consegue (ex: Ficha Detalhada ao tocar no card, Menu com botão Desfazer).
  - **`### ⚡ Melhorias e Refinamentos`:** Recursos que o usuário **já conseguia fazer**, mas que agora ficaram mais rápidos, fáceis ou acessíveis (ex: catálogo integrado mantendo adotados visíveis, botões maiores).
  - **`### 🐛 Correções & Ajustes`:** Conserto de comportamentos que estavam quebrados ou inconsistentes (ex: botão de ficha que não respondia ao toque).
  - **PROIBIÇÃO DE PATCH EM NOVIDADES:** É **estritamente proibido** listar correções de bugs, ajustes simples de layout ou patches na seção de Novidades. Patches pertencem à seção de Melhorias ou Correções.
- **Foco Exclusivo no Usuário Final (Sem Itens de Desenvolvedor ou Infraestrutura):** Release notes e o modal `src/data/novidades.ts` são lidos pelas voluntárias da ONG. É **estritamente proibido** listar alterações internas de desenvolvedor, arquitetura ou infraestrutura (ex: criação ou segregação de schemas PostgreSQL `public`/`piloto`, migrations, variáveis de ambiente, CI/CD, refatorações internas ou otimizações de query). A comunicação deve focar 100% no que mudou, foi consertado ou adicionado para quem usa o app no dia a dia, em linguagem clara, direta e explicativa, sem jargões e sem explicações didáticas longas.
- **Rastreabilidade Técnica Interna (`CHANGELOG_TECNICO.md`):** Para cada entrega ou build de patch, o agente DEVE documentar detalhadamente no arquivo `CHANGELOG_TECNICO.md` o histórico de engenharia: versão, tipo (Feature, Enhancement, Patch), commit/PR, arquivos modificados, causa raiz do problema, solução técnica adotada e testes validados. Enquanto o usuário recebe notas leves e amigáveis, a engenharia mantém controle total para auditoria e prevenção de regressões.
- **package.json:** O agente DEVE sempre verificar e sincronizar o campo `"version"` no `package.json` para corresponder à versão do build/release atual (ex: `0.12.0`).
