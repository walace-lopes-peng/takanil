...
- `supabase/schema.sql` e `supabase/schema_piloto.sql` — **nunca alterar uma tabela existente sem eu pedir explicitamente**. Se a tarefa precisa de um campo novo, pare e pergunte antes de rodar qualquer SQL.
- **Paridade Obrigatória de Schemas (Multi-Tenancy):** O projeto utiliza `public` para testes/dev e `piloto` para dados reais de produção. É **estritamente proibido** aplicar alterações em `public` sem refletir exatamente em `piloto`. Use `supabase db diff --schema public,piloto` para auditar divergências de DDL antes de qualquer release.
...
- **Linguagem Simples e Direta (Sem Termos Técnicos nem Prolixidade):** As notas em `RELEASE_NOTES.md` e no `src/data/novidades.ts` NÃO devem conter jargões técnicos (nomes de variáveis, rotas de API, termos de banco/infraestrutura), nem explicações didáticas longas voltadas para leigos. A comunicação deve ser **estritamente direta, concisa e objetiva**, listando o que há de novo e prático na versão, acompanhado da data de lançamento e do histórico das versões anteriores.
